...
from dataset import DoubleHDF5_DataSet, noise_class2idx, noise_class2weight, NoiseLevelRegDataset
...
def get_args():
    ...
    parser.add_argument('--strength_dir', type=str, default='', help='')
    parser.add_argument('--ynoise_dir', type=str, default='', help='')
...
####数据集准备  联合训练的 dataset对象和之前的不一样

    train_data = DoubleHDF5_DataSet(ynoise_path=args.ynoise_dir, strength_path=args.strength_dir, phase='train', 
                            transform=get_transform(True, args.patch_size, params=args.crop_sel))
    valid_data = DoubleHDF5_DataSet(ynoise_path=args.ynoise_dir, strength_path=args.strength_dir, phase='val', 
                            transform=get_transform(False, args.patch_size, params=args.crop_sel))

    train_dateloader = torch.utils.data.DataLoader(train_data, batch_size=256, shuffle=True, num_workers=4, pin_memory=True)
